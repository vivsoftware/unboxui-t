...
const SaveAddress = () => {
  ...
  useEffect(() => {
    auth.onAuthStateChanged((user) => {
      ...
      setUser(user);
      if (user.email) {
        axios.get(`${spring_boot_url}api/users/email?email=${user.email}&apiKey=${X_API_KEY=rachna}`,{
          headers:{
            "Authorization":rachna
          }
        }
          ...
          .then(resp => {
            ...
            // console.log("firstname", resp.data.lastName);
            // console.log("lastname", resp.data.firstName);
  ...
  //         const updatedUserDe = userDe.filter((item) => item.id !== id);
  //         setUseraddress(updatedUserDe);
  //                 window.location.reload(); // Refresh the page
  ...
  console.log("addresssssssssss", userDe)
                ...
                {Array.isArray(useraddress) && useraddress?.map((item,i) => (
                  <Col xl="4" md="6" key={i}>
                    <div className="save-details">
                      ...
                      {/* <div className='mobile'>
                          <p className='font-light mobile'>{userDe ? (
                          <span>{userDe.phoneNumber} </span>
                  ) : (
                    <span>Please sign in...</span>
                      ...
                      <div className="button">
                        {/* <a href='#javascript' className='btn btn-sm' onClick={() => handleModalupdate()} >
                          Edit
                        </a> */}
